...
public class clientC {
	...
	public static void encodeNonce (byte[] n) {
		try {
			ciph = Cipher.getInstance("RSA/ECB/PKCS1Padding");
			ciph.init(Cipher.ENCRYPT_MODE, priv);
			encryptedNonce = ciph.doFinal(n);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
//				n.nextBytes(nBytes);
				...
				System.arraycopy(encryptedNonce, 0, m, encrypted.length + signed.length, encryptedNonce.length);
